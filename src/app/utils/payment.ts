...
export const initiatePayment = async (paymentData: TCustomerDetails) => {
  try {
    const response = await axios.post(config.payment_url!, {
      ...
      success_url: `http://localhost:5000/api/auth/confirmation?transactionId=${paymentData.tranId}&userId=${paymentData.userId}`,
      fail_url: `http://localhost:5000/api/auth/confirmation?status=failed`,
